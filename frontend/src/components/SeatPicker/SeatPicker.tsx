...
export function Seat_Picker({ className, seatGroups, BookedSeats }: SeatPickerProps) {

    const isSeatBooked = (row: string, col: number): boolean => {
    ...
    const handleSeatClick = (row: string, col: number) => {
        ...
        console.log("Is seat booked?", isSeatBooked(row, col));
    };
                ...
                <div key={i} className="w-full seat-row flex flex-row justify-between mt-4">
                    ...
                    {[...Array(Math.ceil(group.cols / 2))].map((_, j) => (
                        ...
                        className={cn("seat flex seat h-12 w-8 rounded-md text-neutral-200 items-center justify-center bg-zinc-500 hover:bg-zinc-600", {
                            "bg-red-500": isSeatBooked(String.fromCharCode(65 + i), j+1)
                            })}
                        onClick={() => handleSeatClick(String.fromCharCode(65 + i), j + 1)}
                        >
                        {j + 1}
                        </div>
                    ))}
                    ...
                    <div className="w-full seat-group-right flex flex-row justify-evenly">
                        {[...Array(Math.floor(group.cols / 2))].map((_, k) => (
                            ...
                            key={k + 1}
                            className={cn("seat flex seat h-12 w-8 rounded-md text-neutral-200 items-center justify-center bg-zinc-500 hover:bg-zinc-600", {
                                "bg-red-500": isSeatBooked(String.fromCharCode(65 + i), group.cols / 2 + k)
                                })}
                            onClick={() => handleSeatClick(String.fromCharCode(65 + i), group.cols / 2 + k)}
                            >
                            {(group.cols / 2) + k + 1}
                            </div>
                        ))}
